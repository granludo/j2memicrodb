...
public class PDAStoreIndexVector extends PDAStoreIndex {
	...
	/** Posició física del vector que implementa l´índex on es troba l'entrada del primer PDAStoreable del PDALabeledStoreSequence */
	protected int First;
	...
	{		
		...
		if(!entradaaactualitzar.getLabel().equals(p.getLabel())) return false;
	...
	protected boolean delete()
	{
		...
		//Actualització de l'índex
		entrada = updateDelete(entrada, posicio);		
	...
	/**
	 * Sitúa el Current a la primera entrada que coincideix amb l'etiqueta especificada.
	 * @return Cert si s'ha arribat al final del fitxer. 
	 */
	protected boolean seek(String label)
	{
	 ...
	 * @return Posició física del vector on ha d'anar la nova entrada.
	 */
